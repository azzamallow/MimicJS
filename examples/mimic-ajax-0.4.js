...
}Mimic.Ajax=function(){var that=this;
this.data=[];
var matchers=function(url,ajax){return{toHaveResponse:function(status,text){ajax.data.push({url:url,text:text,status:status});
}};
};
this.request=function(url){return matchers(url,that);
};
this.start=function(){XMLHttpRequest=function(ajax){return function(){this.onreadystatechange=undefined;
this.readyState=-1;
...
i<ajax.data.length;
i++){if(ajax.data[i].url===url){this.responseText=ajax.data[i].text;
...
var ajax=Mimic.Ajax;
if(typeof jasmine!=="undefined"){var jasmineIt=jasmine.Env.prototype.it;
jasmine.Env.prototype.it=function(description,func){var toReturn=jasmineIt.call(this,description,function(){try{ajax=new Mimic.Ajax();
ajax.start();
func(ajax.request);
Mimic.verify();
Mimic.reset();
