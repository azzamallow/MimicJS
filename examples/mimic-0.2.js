...
if(callPrefix!=null){callString=[callPrefix,".",callString].join("");
...
if(asPartial==true){instrumentedFunction=instrumentedFunction.concat([" return (",object[member].toString(),")(",Mimic.Util.Parameters.arguments(object[member]),");"]);
}instrumentedFunction=instrumentedFunction.concat(" } }");
eval(instrumentedFunction.join(""));
}else{if(typeof object[member]=="object"&&object[member]!=null&&object[member].join==null){if(!Mimic.isMimic(object[member])){Mimic.Instrument(object[member],asPartial,object,callString);
