...
Mimic.jQuery=mimic();
return mimic;
}else{if(!Mimic.isMimic(object)){Mimic.Instrument(object);
...
window.alert=function(actualMessage){Screw.Matchers.expect(actualMessage).to(Screw.Matchers.equal,message);
};
var thrown,message;
...
Mimic.Util.Object={equals:function(object1,object2){if(typeof object1=="number"&&isNaN(object1)&&typeof object2=="number"&&isNaN(object2)){return true;
...
}else{if(typeof object1[i]=="object"||typeof object1[i]=="function"||isNaN(object1[i])){if(this.equals(object1[i],object2[i])==false){return false;
...
}for(var key in object){if(object[key]==object){newObject[key]=newObject;
}else{if(object[key]&&typeof object[key]=="object"){newObject[key]=Mimic.Util.Object.clone(object[key]);
...
i<this.calls.length;
i++){if(this.calls[i].isValidMimic(mimic)&&Mimic.Util.Array.equals(parameters,this.calls[i].parameters)&&this.calls[i].checked==false){return this.calls[i];
}}return null;
};
};
...
Mimic.Instrument=function(object,parentMimic,callPrefix){for(var member in object){var callString=member;
...
}else{if(typeof object[member]=="object"&&object[member]!=null&&object[member].join==null){Mimic.Instrument(object[member],object,callString);
